# Migration Guide - Forge Engineering Lifecycle Framework

## Overview
This guide provides procedures for migrating from ad-hoc prompting or other frameworks to the Forge Engineering Lifecycle Framework. It covers assessment, planning, and phased migration approaches.
# Migration Guide - Forge Engineering Lifecycle Framework

## Migrating from Ad-hoc Prompting

### Assessment
1. Document current prompting approach
2. Identify which SDLC phases you cover
3. Note existing templates or conventions
4. Identify pain points and gaps

### Migration Steps

| Week | Action |
|------|--------|
| 1 | Replace ad-hoc research with `workflows/01-research.md` |
| 2 | Add `workflows/02-analyze.md` for requirements analysis |
| 3 | Add `workflows/04-implement.md` for coding tasks |
| 4 | Full coverage, build artifact library |

## Migrating from Other Frameworks

| From | Migration |
|------|-----------|
| Waterfall/SDLC | Map phases to Forge workflows, adopt templates |
| Agile/Scrum | Map sprint activities to workflows |
| DevOps/CI-CD | Map pipeline stages to workflows |
| Design Thinking | Map empathize/define/analyze/design to workflows |

## Template Migration

### Converting Existing Templates
1. Map your template sections to Forge template sections
2. Identify missing sections and add them
3. Remove redundant sections
4. Align terminology with Forge glossary

### Creating New Templates
1. Start with relevant Forge template
2. Customize for your domain
3. Add organization-specific sections
4. Validate with quality gates

## Data Migration

### Research Repository
1. Catalog existing research findings
2. Convert to Forge research report format
3. Add source quality assessments
4. Add verification logs

### Artifact Library
1. Catalog existing artifacts
2. Map to Forge artifact types
3. Convert to Forge templates
4. Add metadata (phase, status, version)

## Team Migration

### Training Plan
| Week | Activity |
|------|----------|
| 1 | Framework overview and quick start |
| 2 | Hands-on with one workflow |
| 3 | Expand to multiple workflows |
| 4 | Full adoption and customization |

### Resistance Management
1. Demonstrate value with pilot project
2. Show before/after comparison
3. Address specific concerns
4. Provide hands-on support
5. Celebrate early wins

## Post-Migration Checklist

- [ ] All workflows documented and accessible
- [ ] Team trained on framework conventions
- [ ] Templates customized for organization
- [ ] Quality gates being used consistently
- [ ] Artifacts in standard format
- [ ] Research repository established
- [ ] Configuration files in place
- [ ] Troubleshooting guide accessible
